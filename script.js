...
function colorizeYear(dateStr) {
    return dateStr.replace(/(\d{4})$/, (year) => {
        const colorClass = year === '2026' ? 'text-blue-500 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500';
        return `<span class="${colorClass}">${year}</span>`;
    });
}
...
function renderTable(clubs = getAllClubs()) {
    ...
    tbody.innerHTML = clubs.map((club, index) => {
        ...
        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group ${hasPanel ? 'cursor-pointer' : ''}" data-club-name="${club.name.replace(/"/g, '&quot;')}">
            ...
            <td class="px-4 py-5 text-sm font-bold"><span class="block">${colorizeYear(club.recruitStart)}</span><span class="text-slate-400">→ ${colorizeYear(club.recruitEnd)}</span></td>
        ...
        <${Tag} ${hrefAttr} class="block p-4 bg-white dark:bg-card-dark border border-slate-200 dark:border-border-dark rounded-2xl backdrop-blur-xl bg-opacity-70 shadow-lg" data-club-name="${club.name.replace(/"/g, '&quot;')}">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium">${colorizeYear(club.recruitStart)} → ${colorizeYear(club.recruitEnd)}</span>
                </div>
                <div class="flex items-center gap-2">
